...
def get_data(path):
    ...
    raw_text = raw_text[-n_row:]
    raw_text = [row.split() for row in raw_text]
    data = [list(map(float, row)) for row in raw_text]
    data = np.array(data)
    Y = data[:, -1]
    X = data[:, 1:-1]
    return X, Y
...
class RidgeRegression:
    ...
    def fit_gradient_decent(
        self, X_train, Y_train, LAMBDA, max_num_epoch=100, batch_size=128
    ):

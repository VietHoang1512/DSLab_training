...
def gen_data_and_vocab():
    ...
    train_path, test_path = (
        (parts[0], parts[1]) if "train" in parts[0] else (parts[1], parts[0])
    )
    ...
    vocab = [
        word for word, freq in zip(word_count.keys(), word_count.values()) if freq > 10
    ]
    vocab.sort()
...
def encode_data(data_path, vocab_path):
    with open(vocab_path) as f:
        vocab = dict(
            [(word, word_ID + 2) for word_ID, word in enumerate(f.read().splitlines())]
        )
    with open(data_path) as f:
        documents = [
            (line.split("<fff>")[0], line.split("<fff>")[1], line.split("<fff>")[2])
